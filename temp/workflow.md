...
7. For each payment_type_assignment, insert EITHER accounts OR aliases (not both):
       ...
       Note: Each payment type carries either accounts or aliases, never both.
...
6. For each payment_type_assignment, insert EITHER accounts OR aliases (not both):
